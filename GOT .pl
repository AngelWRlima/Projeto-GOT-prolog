...
unkgender(martell_unk_1).
unkgender(martell_unk_2).
unkgender(martell_unk_3).
unkgender(martell_unk_4).
...
% integrantes:
% Angel Willyan Rodrigues de Lima (awrl)
% Diego Rafael Gomes de França (drgf)
% Joao Lucas Vieira dos Santos (jlvs)
% Renan Martiniano Santana de Lima (rmsl3)

%TAREFA 4
%Predicado que retorna true quando X nao tem filhos, usando not parent
is_single(X) :-
    not(parent(X, Y)),
	Y = none.
...
%TAREFA 2
%primeiro faz as excecoes de quem tem 2 clans ou o nome não bate com nenhum clã
clan(jon_snow, stark).
clan(jon_snow, targaryen).
clan(alerie_hightower, tyrell).d
...
clan(four_unknown_martells, martell).

%Checa a familia de cada um criando uma lista com separada pelo '_'
% Depois checa se na lista tem algum nome de clan
% Se tiver, Y recebe nome do clan
house_of(X, Y) :-
    member(X, [jon_snow, alerie_hightower, mellario_of_norvos, aegon_targaryen, rhaenys_targaryen, alannys_harlaw, gendry])
    ,clan(X, Y)
...
%TAREFA 3
% Usa o setof para criar uma lista com todos de uma casa
% Depois, usa o length para pegar o tamanho da lista com o clan
power_of(X, Y):-
	setof(Z,((male(Z);female(Z);unkgender(Z)), (house_of(Z, Clan),(X = Clan))), Lista),
	length(Lista, Y).


%TAREFA 5
%descobrir quantos aliados um casamento gerará
%Caso os aliados sejam do mesmo clã que a primeira pessoa, eles não serão adicionados
%Caso a segunda pessoa seja do mesmo clã que a primeira pessoa, o programa para

uncle_clan(X, Y, Clan) :-
    uncle(Tio, Y), house_of(Tio, Clan_Tio),not(Clan = Clan_Tio),
    X=Tio.

aunt_clan(X, Y, Clan) :-
    aunt(Tia, Y), house_of(Tia, Clan_Tia), not(Clan = Clan_Tia),
    X=Tia.

nephew_clan(X, Y, Clan) :-
    nephew(Sobrinho, Y), house_of(Sobrinho, Clan_Sobrinho), not(Clan = Clan_Sobrinho),
    X=Sobrinho.

neice_clan(X, Y, Clan) :-
    neice(Sobrinha, Y), house_of(Sobrinha, Clan_Sobrinha), not(Clan = Clan_Sobrinha),
    X=Sobrinha.

ancestor_clan(X, Y, Clan) :-
    ancestor(Ancestral, Y), house_of(Ancestral, Clan_Ancestral), not(Clan = Clan_Ancestral),
    X=Ancestral.

siblings_clan(X, Y, Clan) :-
    sibling(Y, Sis), house_of(Sis, Clan_Sis), not(Clan = Clan_Sis),
    X=Sis.

%função que retorna o números dos aliados
list_aliados(Y,Clan,Ally) :-
    ((setof(Z, ancestor_clan(Z,Y,Clan), A)); A = []), length(A,LenA),% write(A),
    ((setof(X, siblings_clan(X,Y,Clan), B)); B = []), length(B,LenB),% write(B),
    ((setof(W, uncle_clan(W,Y,Clan), C)); C = []), length(C,LenC),% write(C),
    ((setof(V, aunt_clan(V,Y,Clan), D)); D = []), length(D,LenD),% write(D),
    ((setof(U, nephew_clan(U,Y,Clan), E)); E = []), length(E,LenE),% write(E),
    ((setof(T, neice_clan(T,Y,Clan), F)); F = []), length(F,LenF),% write(F),
    Ally is (LenA + LenB + LenC + LenD + LenE + LenF + 1).

%função principal
marriage_power(X, Y, Z) :-
    ((male(X), female(Y)); (female(X), male(Y))),
    is_single(X), is_single(Y),
    house_of(X, A), house_of(Y, B), not(A==B),
    list_aliados(Y,A,Ally), Z is (Ally).
